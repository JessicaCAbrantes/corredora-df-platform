...
Idempotency by webhook `event.id` (ledger) is implemented in FASE 3.4-C1/C2. Concurrency hardening (conditional updates, stale checkout, one PENDING payment) is FASE 3.4-C3. HTTP retry policy (signature→401; permanent allowlist→200+PROCESSED; `PAYMENT_NOT_FOUND`/transient→500+RECEIVED) is FASE 3.4-C4. **Ops runbooks (rotation, troubleshooting, production checklist):** [`docs/ops/payments-runbook.md`](../ops/payments-runbook.md) (FASE 3.4-D). Rotation is **hard cutover** (single active secret; no dual-secret / zero-downtime in the app today).
